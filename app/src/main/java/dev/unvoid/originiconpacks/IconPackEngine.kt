...
class IconPackEngine(private val context: Context) {
    ...
    private fun aospThemedBitmap(drawable: Drawable, size: Int, packageName: String, tint: Int?, inverted: Boolean, scale: Float): Bitmap {
        ...
        val backgroundColor = tint?.let { ColorUtils.blendARGB(baseBackground, it, if (inverted) 0.22f else 0.16f) } ?: baseBackground
        val foregroundColor = tint?.let { ColorUtils.blendARGB(baseForeground, it, if (inverted) 0.34f else 0.26f) } ?: baseForeground
